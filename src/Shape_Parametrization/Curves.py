...
class CatmullRom_curve:

    Characteristic_mat = (1 / 2) * np.array([[0, 2, 0, 0], [-1, 0, 1, 0], [2, -5, 4, -1], [-1, 3, -3, 1]])
    ...
    def generate_points(self, total_points):
        t = np.linspace(0, 1, total_points)
        T_mat = np.vstack((t ** 0, t, t ** 2, t ** 3)).T
        generated_points = T_mat @ CatmullRom_curve.Characteristic_mat @ self.ControlPoints_mat
        return generated_points
...
class B_Spline_curve:
    ...
    Characteristic_mat = (1 / 6) * np.array([[1, 4, 1, 0], [-3, 0, 3, 0], [3, -6, 3, 0], [-1, 3, -3, 1]])
    ...
    def generate_points(self, total_points):
        t = np.linspace(0, 1, total_points)
        T_mat = np.vstack((t ** 0, t, t ** 2, t ** 3)).T
        generated_points = T_mat @ B_Spline_curve.Characteristic_mat @ self.ControlPoints_mat
        return generated_points
