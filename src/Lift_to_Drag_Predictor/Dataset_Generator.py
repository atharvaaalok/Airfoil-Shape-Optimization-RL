...
numpy_arr_file_path = directory_path + '/Dataset/Arrays_as_rows.txt'
rewards_file_path = directory_path + '/Dataset/Rewards_as_rows.txt'
    ...
    for iteration in range(max_iterations):
        ...
        for i_N in range(N):
            ...
            for t in range(T):
                ...
                # Generate an action
                a = np.zeros(s.shape)
                a[idx_to_change, :] = np.random.rand(len(idx_to_change), 2) / 1000
