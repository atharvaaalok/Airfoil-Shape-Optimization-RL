...
def make_env(s0, idx_to_change, max_iterations, a_scaling, valid_states_file_path) -> gym.Env:
...
if __name__ == '__main__':
    ...
    if parallelize:
        ...
        training_env = make_vec_env(lambda: make_env(s0, idx_to_change, MAX_ITERATIONS, a_scaling, valid_states_file_path), n_envs = num_cpu, vec_env_cls = SubprocVecEnv)
    else:
        training_env = CFD_Env(s0, idx_to_change, MAX_ITERATIONS, a_scaling, valid_states_file_path)
        # Reset the environment
